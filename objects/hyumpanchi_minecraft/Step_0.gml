...
if hp > 0 && cannot_move = 0 && hit_motion = 0 && hit_cooltime = 0
{
	...
	attack_dilay = 1
		...
		{
		random_attack_chance = choose(1,1,1,0,0,0,0,0,0)
		}
		else
		{
...
t_x = x
	...
	var s_hspeed = -image_xscale*abs(player.x-x)/8
	
	if abs(s_hspeed) > 9
	{
	s_hspeed = -image_xscale*9
	}
	...
	bomb.hspeed = s_hspeed
	bomb.vspeed = s_hspeed
	bomb.image_xscale = 0.75
	bomb.image_yscale = 0.75
	bomb.image_index = choose(0,3)
	...
	var _ef = instance_create_depth(x,y,depth+1,effect_punch_attack)
