...
if global.n_room = 0
{
	if jang_sa_x != -1
	{
	instance_destroy(jang_sa_x)
	}
jang_sa_x = -1
}
else
{
	if instance_number(player_message) > 0
	{
		if jang_sa_x != -1
		{
		instance_destroy(jang_sa_x)
	...
	if global.killed_wakchang_before = 1
	{
		if hp < 32 && hp > 2
		{
			if jang_sa_x = -1
			{
			jang_sa_x = instance_create_depth(x,y,player.depth-1,player_message)
			jang_sa_x.text = "다시 장사함ㅎ"
			jang_sa_x.target = id
			alarm[0] = 180
...
if hp <= 0
{
	if global.achievement_finish[8] = 0 && instance_number(achievement) = 0 && global.playing_scene = 0
	{
	var _message = instance_create_depth(-100,-100,-9999999999,achievement)
	_message.title = "곱창난 민심"
	_message.text = "왁창을 쓰러뜨렸다"
	global.achievement_finish[8] = 1
	}
}
...
global.killed_wakchang_before = 1
}
